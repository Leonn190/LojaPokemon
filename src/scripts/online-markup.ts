...
export const productVisualMarkup = (item: any, interactive = true) => {
  ...
  return `<div class="collectible-visual ${visualClass}"${isCard ? ` data-foil-tier="${rarityTier(item.type)}"` : ''}${interactive ? ' data-tilt data-tilt-strength="4"' : ''}><div class="image-stage"><span class="image-fallback" aria-hidden="true"><b>${escapeHtml(initials || 'TCG')}</b><small>${escapeHtml(label)}</small></span>${image}${isCard ? '<span class="holo-band" aria-hidden="true"></span><span class="foil-spectrum" aria-hidden="true"></span><span class="glare" aria-hidden="true"></span>' : ''}</div></div>`;
};
