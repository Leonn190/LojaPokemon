...
Na API corrigida, uma consulta MYP bloqueada no Render tenta em ordem: Node direto, sessão/cookie, Python `requests` com os mesmos headers de `teste_myp.py`, Reader normal, Reader em modo browser/no-cache e, se necessário, `cf-browser-rendering`. O erro `MYP_ANTIBOT_CHALLENGE` passa a significar explicitamente que o conteúdo recebido foi a tela `Just a moment...`, e não uma falha do parser da carta.
